...
#python scripts/GenBankSequenceSubmitter.py -q test_data/fasta-seq/ -m test_data/metadata.tsv -n test_data/template.sbt -gff tmp/Gff/NC_001542.gff3 -db gdb.db
...
class GenBankSequenceSubmitter:
	...
	def extract_ref_seq(self):
		...
		cursor.execute("SELECT primary_accession FROM meta_data where accession_type='reference'")
		ref_accs = cursor.fetchall()
		ref_accs_list = [item[0] for item in ref_accs]

		for each_acc in ref_accs_list:
			cursor.execute("SELECT sequence FROM sequences WHERE header = ?", (each_acc,))
			result = cursor.fetchone()
			if result:
				sequence = result[0]
				write_file.write(">" + each_acc)
				write_file.write("\n")
