...
# from models.read_file import read_file
...
class BlastAlignment:
	...
	def __init__(self, query_fasta, db_fasta, tmp_dir, output_file, master_acc):
		self.query_fasta = query_fasta
		self.db_fasta = db_fasta
		self.tmp_dir = tmp_dir
		self.output_file = output_file
	...
	def run_blastn(self):
		print(join(self.tmp_dir, "DB", self.db_file_name))
		
		command = [
			'blastn',
			'-query', self.query_fasta,
			'-db', join(self.tmp_dir, "DB"),
			'-task', 'blastn',
			'-max_target_seqs', '100',
			'-max_hsps', '1',
			'-out', join(self.tmp_dir,"query_tophits.tsv"),
	...
	def process_non_segmented_virus(self, input_file):
		print(self.tmp_dir)
		# self.tmp_dir = "/Users/dana/CVR/backend/models/tmp/"
		# input_file = join("/Users/dana/CVR/backend/models/tmp/query_tophits.tsv")
		...
		seq_dicts = {}
		query_seqs = fasta(self.query_fasta)
		...
		seq_dicts = {}
		print(join(sorted_all, "query_seq.fa"))
	...
	def process_segment_virus(self, input_file, uniq_hit_output, segment_file, annotated_output):
		...
		fasta_seqs = fasta(self.query_fasta)
		...
		for segment, ref_acc in segment_dictionary.items():
			...
			seq_dict = {}
			segment_fa = fasta(join(segment_sorted_all, f'seg_{segment}.fa'))
		...
		reference_seqs = fasta(self.db_fasta)
